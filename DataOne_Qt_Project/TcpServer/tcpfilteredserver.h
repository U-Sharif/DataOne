#ifndef TCPFILTEREDSERVER_H
#define TCPFILTEREDSERVER_H
...
// TcpFilteredServer runs in the main thread.
// This server handles filtered tag records.
// This class inherits QTcpServer of Qt framework, which provides functionality for making TcpServers.

class TcpFilteredServer : public QTcpServer
{
...
public:
    // Constructor.
    explicit TcpFilteredServer(QObject *parent = 0);
...
private:
    ...
    TcpTelnetClient *newFilteredClient;

    // Pointers for creating server connection threads.
    // Server connection threads only send filtered records.
    TcpServerConnection *Timing1ServerConnection;
    TcpServerConnection *Timing2ServerConnection;

    // Functions for setting up server connections.
    void SetupTimingServer1();
    void SetupTimingServer2();
...
private slots:
    void IncrementConnectedClientsCount();          // Slot to increase the connected clinets count on a client thread creation.
